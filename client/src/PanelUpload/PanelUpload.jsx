...
export function PanelUpload({ service }) {
  const [activeTab, setActiveTab] = useState('ip')
  const [fileList, setFileList] = useState([]) // <-- этот список передаем в ProgressTracker
  const [uploading, setUploading] = useState(false)
  const [progressVisible, setProgressVisible] = useState(false)
  const [isProgressMinimized, setIsProgressMinimized] = useState(false)
  const [currentClientId, setCurrentClientId] = useState(null)
  ...
  const handleUpload = async (files) => {
       ...
       // Сохраняем список файлов для передачи в ProgressTracker
    setFileList(files)
    ...
    if (processStateRef.current.isActive && processStateRef.current.sessionId) {
      console.log(
        ...
        processStateRef.current.sessionId
      )
        ...
        >
          {activeTab === 'json' ? (
            <JSONUploadProgress
              ...
              isMinimized={false}
              isModal={true}
              // fileList={fileList} // Передаем список файлов
            />
          ) : (
            <ProgressTracker
