...
export const getPortDataTable = async (req, res) => {
  try {
    // Получаем все записи из таблицы WellKnownPort
    const wellKnownPorts = await WellKnownPort.findAll({
      attributes: ["port", "name"],
      order: [["port", "ASC"]],
    });

    // Формируем массив port_data
    const port_data = wellKnownPorts.map((wellKnownPort) => ({
      port: wellKnownPort.port,
      name: wellKnownPort.name,
    }));

    return res.json({ port_data });
  } catch (error) {
    console.error("Ошибка в getPortDataTable:", error);
    return res
      .status(500)
      .json({ error: "Нет результатов удовлетворяющих поиску" });
  }
